<?php
/**
 * Kâğıt teması — haber detay.
 *
 * BİK / Basın Kanunu m.4 (docs/BIK-NOTLARI.md §3):
 * yayım tarihi VE — farklıysa — güncelleme tarihi içeriğin üzerinde basılır.
 * Gövdenin ilk harfi "drop-cap" olarak büyütülür (theme_setting('drop_cap')).
 */
...
<div class="detay iki-sutun">
  <article class="haber ana-sutun" itemscope itemtype="https://schema.org/NewsArticle">

    <?= breadcrumbs($yol) ?>

    <header class="haber-ust">
      <p class="haber-kicker">
        <?php if (!empty($post['is_breaking'])): ?><span class="rozet-son-dakika">SON DAKİKA</span><?php endif; ?>
        <?php if (!empty($post['category_name'])): ?>
          <a class="haber-kategori" href="<?= esc(url('kategori/' . $post['category_slug'])) ?>"><?= esc($post['category_name']) ?></a>
        <?php endif; ?>
      </p>
    ...
    <?php if ($ilgili): ?>
      <section class="ilgili">
        <hr class="cizgi-kalin">
        <h2 class="sutun-baslik">İlgili Haberler</h2>
