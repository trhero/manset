  ...
  <div class="ana-sutun">
    <?php foreach ($blocks as $b):
      ...
      switch ($tip):
        ...
        // ---------------------------------------------------------- son dakika
        // Şerit parts/header.php içinde basılır; blok açıkken anasayfada
        // saat damgalı ayrıntılı liste gösterilir.
        case 'breaking':
          $items = breaking($limit ?: 5);
          if (!$items) { break; } ?>
          <section class="blok bulten-serit">
            <h2 class="blok-baslik"><?= esc($baslik !== '' ? $baslik : 'Son Dakika') ?></h2>
            <ul class="tarih-liste">
              <?php foreach ($items as $p): ?>
                <li>
                  <time datetime="<?= esc(arr($p, 'published_at', '')) ?>"><?= esc(date('d.m.Y H:i', strtotime((string)arr($p, 'published_at', 'now')))) ?></time>
        ...
        case 'latest':
          ...
          foreach ($items as $it) { $gosterilen[] = (int)$it['id']; } ?>
          <section class="blok">
            <h2 class="blok-baslik"><?= esc($baslik !== '' ? $baslik : ($bolge !== '' ? $bolge . '\'dan Son Haberler' : 'Son Haberler')) ?></h2>
            <div class="izgara">
          ...
          foreach ($cats as $cat):
            ...
            <section class="blok kategori-blok" style="--kat:<?= esc($cat['color']) ?>">
              <h2 class="blok-baslik">
                ...
                <a class="tumu" href="<?= esc(url_category($cat)) ?>">Tümü →</a>
              </h2>
              <div class="izgara">
                <?php foreach ($items as $p) { part('card', ['post' => $p, 'boyut' => 'orta']); } ?>
              </div>
        ...
        case 'weather':
          ...
          if (!$hava) { break; } ?>
          <section class="blok veri-serit hava">
