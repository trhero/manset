  ...
  <article class="ana-sutun haber" itemscope itemtype="https://schema.org/NewsArticle">
    ...
    <?php if ($img): ?>
      <figure class="haber-gorsel">
        <picture>
          <?php if ($webp !== ''): ?>
            <source type="image/webp" srcset="<?= esc($webp) ?>" sizes="(max-width:1000px) 100vw, 780px">
          <?php endif; ?>
          <?php /* LCP adayı: sayfanın en büyük görseli, öncelikli yüklenir. */ ?>
          <img src="<?= esc($img) ?>" <?= $srcset ? 'srcset="' . esc($srcset) . '" sizes="(max-width:1000px) 100vw, 780px"' : '' ?>
    ...
    <?php if (!empty($related)): ?>
      <section class="blok">
        ...
        <div class="izgara dar"><?php foreach ($related as $p) { part('card', ['post' => $p, 'boyut' => 'kucuk']); } ?></div>
      </section>
    <?php endif; ?>
