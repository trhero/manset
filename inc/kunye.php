...
if (!defined('KUNYE_ANSWER_DEADLINE')) { define('KUNYE_ANSWER_DEADLINE', 86400); }   // yanıt süresi: 1 gün
if (!defined('KUNYE_DEADLINE_WARN'))   { define('KUNYE_DEADLINE_WARN', 21600); }     // 6 saat kala uyarı
...
/**
 * Yanıt süresi durumu — `created_at + 1 gün` (docs/BIK-NOTLARI.md §4).
 *
 * kalan > 6 saat            → zamaninda / olumlu
 * 0 < kalan <= 6 saat       → yaklasiyor / uyari
...
function corrections_deadline_state(array $row) {
    ...
    $deadline = $created + KUNYE_ANSWER_DEADLINE;
...
function corrections_list(array $filters = [], $limit = 30, $offset = 0) {
    ...
    return qa('SELECT c.id, c.post_id, c.name, c.email, c.message, c.note, c.answer, c.ip, c.status,
                      c.created_at, c.publish_at, c.homepage_until, c.visible_until,
                      p.title AS post_title, p.slug AS post_slug
               FROM corrections c LEFT JOIN posts p ON p.id = c.post_id'
             . $where . ' ORDER BY c.id DESC LIMIT ' . $limit . ' OFFSET ' . $offset, $p);
}
...
function corrections_counts() {
    $out = ['pending' => 0, 'answered' => 0, 'rejected' => 0, 'all' => 0];
...
function corrections_overdue_count() {
    $cut = date('Y-m-d H:i:s', time() - KUNYE_ANSWER_DEADLINE);
